...
public class Transporte {
	...
	void adquirirBillete()  throws IOException{
		...
		if (tipo >=1 && tipo <=2) {
		  ...
		  int numero=(int)f.length()/b.getTamanoRegistro();
  	      b=new Billete(tipo, hoy.agno, hoy.mes,hoy.dia,hoy.hora, hoy.minuto, 1,dni);
  	      f.seek(f.length());
		  b.escribir(f);
		}
		else 
	...
	void detectarIrregularidades()  throws IOException {
	  ...
	  if (fv.exists()) {
		...
		while (br.ready()) {
			...
			b.leer(f);
			correcto=false;
			if (b.tipo==1 && b.ano==fViaje.agno)
			  correcto=true;
			if (b.tipo==2 && 
			     new Fecha(b.ano,b.mes, b.dia, b.hora,b.minuto).menor90minutos(fViaje))  
			  correcto=true;
			if (!correcto) {
			  bw.write(b.dni+"#"+b.numero+"#"+fViaje.agno +"#"+fViaje.mes
	...
	void billetesCliente() throws IOException{
		...
		while (hayDatos){
			...
			hayDatos=b.leer(f);
			if(hayDatos)
			  fBillete= new Fecha(AÑO, b.mes, b.dia,b.hora,b.minuto);
		}
